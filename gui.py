...
class Application(tk.Frame):
    ...
    def create_menu(self):
        ...
        file_menu.add_command(label="Solve", command=self.solve_and_visualize)  # New "Solve" button
    ...
    def solve_and_visualize(self):
        if self.level_file_name:
            solution = solve_sokoban_elem(self.warehouse)
            if solution == 'Impossible':
                print("\nSolution: Impossible")
            else:
                print("\nSolution found:")
                print(solution)
                # Iterate through each step in the solution and visualize it
                self.visualize_solution(solution, 0)

    def visualize_solution(self, solution, index):
        if index < len(solution):
            step = solution[index]
            self.move_player(step)
            self.master.after(500, self.visualize_solution, solution, index + 1)  # Non-blocking delay
